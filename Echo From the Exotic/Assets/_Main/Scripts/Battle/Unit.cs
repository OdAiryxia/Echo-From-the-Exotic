...
public class Unit : MonoBehaviour
{
    ...
    public void TakeDamge(float damage, bool isCrit)
    {
        BattleGameManager.instance.GenerateDamagePopup(((int)damage), this.transform.position, isCrit);
        health -= ((int)damage);
    ...
    public (float damage, bool isCrit) CritDamage()
    {
        bool isCrit = critRate >= Random.Range(0f, 100f);
        float damage = isCrit ? attack * ((100 + critDamage) / 100) : attack;
